---
name: create-pr
description: Open a pull request from the branch you're already on, with an auto-generated title and description. Use whenever the user wants a PR for an existing pushed branch — including paraphrases like "create PR", "open a pull request", "raise a PR", "submit for review", "put this up for review". Prefer this skill over running git/gh commands manually whenever a pull request is requested. If the work still needs its own branch first, use create-branch-and-pr instead.
model: sonnet
